...
class BaseBuildCommand(Command):
    ...
    def _setup_js_deps(self):
        ...
        try:
            node_version = (
                self._run_command(["node", "--version"]).decode("utf-8").rstrip()
            )
        except OSError:
        ...
        if node_version[2] is not None:
            ...
            self._run_yarn_command(["install"])
            self._run_yarn_command(["build"])
...
VERSION = os.environ.get("HYAKUMORI_VERSION", "0.1.0.dev0")
...
setup(
    ...
    author="DataFluct Team",
    url="https://gitlab.com/datafluct/hyakumori_crm",
