...
def refresh_customer_forest_cache(forest_ids: List[str]):
    ...
    forestcustomers = ForestCustomer.objects.filter(**filters).select_related(
        "customer", "forest"
    )
    ...
    for fc in forestcustomers.iterator():
        try:
            ...
            self_contact_qs = fc.customer.customercontact_set.filter(
                is_basic=True
            ).first()
            ...
            logger.warning(
                f"could not saving latest user self contact info in forest: {fc.forest.pk}",
                exc_info=e,
            )
