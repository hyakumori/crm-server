...
export default {
  ...
  async created() {
    try {
      const response = await this.$rest.get("/postal-histories/headers");
  ...
  methods: {
    ...
    async onSearchArchives() {
      ...
      const api_url = `/postal-histories?page_size=${this.options.itemsPerPage}&${this.filterQueryString}`;
    ...
    rowData(val) {
      this.$router.push(`/postal-histories/${val}`);
    },
    ...
    paginationHandler(val) {
      if (val.page > this.currentPage) {
        ...
        const api_url = `/postal-histories?page_size=${val.itemsPerPage}&${this.filterQueryString}`;
    ...
    async updateTagForSelectedArchives() {
      ...
      try {
        ...
        await this.$rest.put("/postal-histories/ids/tags", params);
      } catch (e) {
    ...
    selectedAction(index) {
      switch (index) {
        case 0:
          ...
          this.getSelectedObject("/postal-histories/ids");
          break;
        default:
          return;
      }
